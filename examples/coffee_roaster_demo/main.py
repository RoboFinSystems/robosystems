...
Sets up a complete demo environment for a synthetic DTC + wholesale coffee
roaster: CoA→rs-gaap mappings, depreciation/prepaid schedules, accounting
policy documents, and a filed annual report. The company is profitable but
cash-poor — cash drains into green-coffee inventory and one slow-paying
wholesale account — so an unscripted AI analysis surfaces a real
working-capital reveal. After running, use Claude Desktop or MCP tools to
run the close and analyze the books.

The arc is authored as flow parameters by the scenario engine
(``examples/_scenario``); see ``data.py`` for the Driftline scenario and run
``uv run python -m examples.coffee_roaster_demo.data`` for the offline arc.
...
SOURCE = "native"
CONNECTION_ID = "demo_driftline"
CREATED_BY = "demo:driftline"
CREDENTIALS_FILE = Path(".local/config.json")
DEMO_NAME = "coffee_roaster_demo"
...
def create_demo_graph(entity_type: str = SCENARIO.entity_type) -> str:
  ...
  context = CredentialContext(
    ...
    force=False,
    default_name_prefix="Driftline Demo",
    default_email_prefix="coffee_roaster_demo",
    api_key_prefix="Driftline Demo Key",
    display_title="Driftline Coffee Roasters Demo Setup",
  )
  credentials = ensure_user_credentials(context)
  api_key = credentials["api_key"]

  demo_slot = (
    DEMO_NAME if entity_type == "corporation" else f"{DEMO_NAME}_{entity_type}"
  )
  ...
  request = CreateGraphRequest(
    ...
    tags=["demo", "driftline", "roboledger", "coffee-roaster", entity_type],
  )
...
def create_chart_of_accounts(graph_id: str) -> tuple[dict[str, str], str, int]:
  ...
  envelope = client.create_taxonomy_block(
    graph_id,
    {
      ...
      "description": "Tenant chart of accounts for Driftline Coffee Roasters.",
...
def initialize_fiscal_calendar(graph_id: str) -> str:
  ...
  from .data import MONTHS, get_demo_start_date
  ...
  demo_start = get_demo_start_date(MONTHS)
  ...
  result = client.initialize_ledger(
    ...
    note="coffee_roaster_demo initialization",
  )
...
def create_schedules(graph_id: str, element_lookup: dict[str, str]) -> int:
  ...
  """
  from examples._scenario.engine import add_months as demo_add_months
  from examples._scenario.engine import get_demo_start_date

  from .data import MONTHS

  demo_start = get_demo_start_date(MONTHS)
  taxonomy_name = "Driftline Schedules"
...
def generate_annual_report(graph_id: str) -> str | None:
...
def main() -> None:
  ...
  print(f"\n{COMPANY_NAME} — RoboLedger Showcase Demo Setup")
  print("=" * 60)
  ...
  report_id = generate_annual_report(graph_id)
  ...
  print("    5. Ask: 'We look profitable — show me the income statement.'")
  print("    6. Ask: 'But did cash go up or down over the period? Where did it go?'")
  print("    7. Ask: 'Break down accounts receivable by customer and age it.'")
  print("    8. Ask: 'How much cash is tied up in inventory vs. receivables?'")
  print("=" * 60)
