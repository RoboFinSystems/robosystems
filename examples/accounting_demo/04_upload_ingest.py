...
def main():
    ...
    try:
        ...
        nodes_uploaded = upload_directory(extensions, graph_id, NODES_DIR, "Phase 1 - Nodes")
        rels_uploaded = upload_directory(extensions, graph_id, RELATIONSHIPS_DIR, "Phase 2 - Relationships")

        total_uploaded = nodes_uploaded + rels_uploaded

        if total_uploaded > 0:
            print(f"\n{'=' * 70}")
            print(f"✅ Upload Complete: {total_uploaded} files ({nodes_uploaded} nodes, {rels_uploaded} relationships)")
            print("=" * 70)
