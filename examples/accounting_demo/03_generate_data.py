...
class AccountingDataGenerator:
    ...
    def _create_transaction(self, identifier, date, description, transaction_type, amount=None):
    ...
    def generate_chart_of_accounts(self):
        ...
        for account_number, account_name, account_type, normal_balance in accounts:
            account_data.append({
                ...
                "period_type": "instant" if account_type in ["Asset", "Liability", "Equity"] else "duration",
                "type": "Monetary",
    ...
    def generate_transactions(self):
        ...
        for month in range(self.num_months):
            ...
            line_items.extend([
                ...
                {
                    **self._create_line_item(f"LI{li_id+1:04d}", 0.0, 2500.00, "Cash payment"),
                    "transaction_id": f"TX{tx_id:04d}",
            ...
            for week in range(4):
                ...
                line_items.extend([
                    {
                        **self._create_line_item(f"LI{li_id:04d}", revenue_amount, 0.0, "Cash received"),
                        "transaction_id": f"TX{tx_id:04d}",
                        "account_name": "Assets:Cash",
                        "account_element": f"element_{account_lookup['Assets:Cash']}",
                    },
                    {
                        **self._create_line_item(f"LI{li_id+1:04d}", 0.0, revenue_amount, "Consulting revenue earned"),
                        "transaction_id": f"TX{tx_id:04d}",
            ...
            line_items.extend([
                {
                    **self._create_line_item(f"LI{li_id:04d}", salary_amount, 0.0, "Employee salaries"),
                    "transaction_id": f"TX{tx_id:04d}",
                    "account_name": "Expenses:Salaries",
                    "account_element": f"element_{account_lookup['Expenses:Salaries']}",
                },
                {
                    **self._create_line_item(f"LI{li_id+1:04d}", 0.0, salary_amount, "Cash payment"),
                    "transaction_id": f"TX{tx_id:04d}",
                    "account_name": "Assets:Cash",
                    "account_element": f"element_{account_lookup['Assets:Cash']}",
                }
            ...
            for expense_account, desc, min_amt, max_amt in expense_types:
                ...
                line_items.extend([
                    ...
                    {
                        **self._create_line_item(f"LI{li_id+1:04d}", 0.0, amount, "Cash payment"),
                        "transaction_id": f"TX{tx_id:04d}",
                        "account_name": "Assets:Cash",
                        "account_element": f"element_{account_lookup['Assets:Cash']}",
                    }
        ...
        li_df_clean = li_df.drop(columns=['transaction_id', 'account_name', 'account_element'])
    ...
    def generate_reports_and_facts(self, tx_df, li_df):
        ...
        for month_idx in range(self.num_months):
            ...
            element_totals = defaultdict(lambda: {"debits": 0.0, "credits": 0.0, "element_info": {}})
        ...
        facts_df_clean = facts_df.drop(columns=['report_id', 'element_id', 'period_id', 'unit_id', 'entity_id', 'fact_name'])
    ...
    def generate_all(self, regenerate=False):
        ...
        reports_df, facts_df, periods_df, units_df = self.generate_reports_and_facts(tx_df, li_df)
    ...
    def _files_exist(self):
        ...
        return (
            all((self.nodes_dir / f).exists() for f in node_files)
            and all((self.relationships_dir / f).exists() for f in relationship_files)
        )
