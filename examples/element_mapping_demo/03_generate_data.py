#!/usr/bin/env python3
"""
Generate Element Mapping Demo Data (Parquet Files)

This script generates transaction-based accounting data in Parquet format,
ready for upload to RoboSystems.

NOTE: This demo focuses on TRANSACTIONS ONLY (no pre-computed facts).
The view engine will aggregate transactions into trial balance on-demand,
then apply element mappings to show CoA → US-GAAP aggregation.
...
class AccountingDataGenerator:
  ...
  def generate_all(self, regenerate=False):
    ...
    print("\n" + "=" * 70)
    print("📊 Element Mapping Demo - Data Generation")
    print("=" * 70)
    print(f"Generating {self.num_months} months of transaction data...")
    print("NOTE: This demo uses TRANSACTIONS ONLY (no pre-computed facts)")
    print("      Views will aggregate transactions on-demand via trial balance")

    self.generate_entity()
    self.generate_chart_of_accounts()
    tx_df, li_df = self.generate_transactions()
    self.generate_relationships(tx_df, li_df)
    ...
    print(f"  - {len(li_df)} line items (double-entry journal entries)")
    print("\n💡 Transaction-based data ready for:")
    print("   - Trial balance aggregation")
    print("   - Element mapping application")
    print("   - View generation with mappings")
