...
published XBRL conceptual model and now maintained natively. v1 is intentionally tiny — two abstract concepts,
...
- **`packages[]`** — atomic units owned by this framework. Each entry is
  ...
  - `tenant_copy` (default `true`) is an **orthogonal** axis to
    ...
    **does not copy it into per-tenant schemas**. Use it for content that
    ...
    to `true` and shipping a migration that calls
    `writer.resync_library_into_tenant` (as `0030_rs_gaap_split_repair` does; the copy
