    ...
    response = ssm.send_command(
      ...
      Parameters={
        "commands": [
          ...
          "    ",
          "    # Get the device",
          "    DEVICE=$(df /mnt/ladybug-data | tail -1 | awk '{print $1}')",
          '    echo "Device detected: $DEVICE"',
          "    ",
          "    # Get current and available size",
          "    CURRENT_SIZE=$(df -BG /mnt/ladybug-data | tail -1 | awk '{print $2}' | sed 's/G//')",
          ...
          "    if sudo xfs_growfs /mnt/ladybug-data 2>/dev/null; then",
          '      echo "XFS filesystem grown successfully"',
          "      df -h /mnt/ladybug-data",
          "      return 0",
