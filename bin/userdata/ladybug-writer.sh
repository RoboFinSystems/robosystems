#!/bin/bash
# LadybugDB Writer Instance UserData Script (Refactored - Using Shared Components)
...
LOG_FILE="/var/log/ladybug-writer-setup.log"

# Logging setup - append to the setup log
exec > >(tee -a "$LOG_FILE")
exec 2>&1
echo "Starting LadybugDB writer setup at $(date)"
...
: ${LBUG_NODE_TYPE:?"LBUG_NODE_TYPE variable must be set"}
...
DATABASES=""
if [ "${LBUG_NODE_TYPE}" = "shared_master" ]; then
...
mkdir -p /mnt/ladybug-data/databases/lance
...
chmod 775 /mnt/ladybug-data/databases
...
aws ec2 create-tags \
  ...
  --tags \
    Key=Name,Value="robosystems-ladybug-${LBUG_NODE_TYPE}-${ENVIRONMENT}-root" \
    Key=Environment,Value=${ENVIRONMENT} \
    Key=Service,Value=RoboSystems \
    Key=Component,Value=LadybugWriter \
    Key=NodeType,Value=${LBUG_NODE_TYPE} \
...
# Tag data volume
if [ "${LBUG_NODE_TYPE}" = "shared_master" ] || [ "${LBUG_NODE_TYPE}" = "shared_replica" ]; then
  DATA_VOLUME_NAME="robosystems-ladybug-shared-${ENVIRONMENT}-data"
else
  DATA_VOLUME_NAME="robosystems-ladybug-${LBUG_NODE_TYPE}-${ENVIRONMENT}-data"
fi
...
aws ec2 create-tags \
  ...
  --tags \
    ...
    Key=Component,Value=LadybugWriter \
    Key=NodeType,Value=${LBUG_NODE_TYPE} \
    Key=VolumeType,Value=LadybugData \
    Key=InstanceId,Value=${INSTANCE_ID} \
...
cat > /etc/systemd/system/ladybug-lifecycle.service << EOF
[Unit]
Description=LadybugDB Instance Lifecycle Manager
...
Restart=always
RestartSec=30
Environment="DATABASE_TYPE=ladybug"
Environment="NODE_TYPE=${LBUG_NODE_TYPE}"
...
systemctl enable ladybug-lifecycle.service
systemctl start ladybug-lifecycle.service
...
# ==================================================================================
echo "Starting LadybugDB writer container..."
...
cat > /usr/local/bin/ladybug-health-check-wrapper.sh << 'EOF'
#!/bin/bash
...
sed -i "s/\${LBUG_NODE_TYPE}/${LBUG_NODE_TYPE}/g" /usr/local/bin/ladybug-health-check-wrapper.sh
sed -i "s/\${LBUG_PORT}/${LBUG_PORT}/g" /usr/local/bin/ladybug-health-check-wrapper.sh
sed -i "s/\${ENVIRONMENT}/${ENVIRONMENT}/g" /usr/local/bin/ladybug-health-check-wrapper.sh
sed -i "s/\${AWS_REGION}/${REGION}/g" /usr/local/bin/ladybug-health-check-wrapper.sh
sed -i "s|\${VALKEY_URL}|${VALKEY_URL:-}|g" /usr/local/bin/ladybug-health-check-wrapper.sh
sed -i "s|\${ECR_URI}|${ECR_URI}|g" /usr/local/bin/ladybug-health-check-wrapper.sh
sed -i "s|\${ECR_IMAGE_TAG}|${ECR_IMAGE_TAG}|g" /usr/local/bin/ladybug-health-check-wrapper.sh
...
# COMPLETION
# ==================================================================================
echo "✅ LadybugDB writer setup completed successfully at $(date)"
