...
def mock_cache():
    ...
    with patch(
        "robosystems.security.auth_protection.rate_limit_cache", FakeCache()
    ):
        store.clear()
        yield store
...
def mock_audit_logger():
    ...
    with patch(
        "robosystems.security.auth_protection.SecurityAuditLogger"
    ) as mock:
...
class TestCacheFailureResilience:
    ...
    def test_record_attempt_survives_cache_failure(self):
        with patch(
            "robosystems.security.auth_protection.rate_limit_cache"
        ) as mock_cache:
