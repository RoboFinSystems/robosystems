...
from robosystems.kuzu_api.routers.databases.schema import (
  validate_ddl_statement,
  escape_identifier,
)
from robosystems.kuzu_api.app import create_app
from robosystems.kuzu_api.core import init_cluster_service
...
class TestSchemaEndpointSecurity:
  ...
  def test_path_traversal_in_database_name(self, test_client):
    ...
    with patch(
      "robosystems.kuzu_api.core.cluster_manager.get_cluster_service"
    ) as mock_get_service:
      mock_service = Mock()
  ...
  def test_sql_injection_in_schema_endpoint(self, test_client):
    ...
    with patch.object(
      ...
      with patch.object(cluster_service, "execute_query", return_value=mock_result):
        ...
        with patch(
          "robosystems.kuzu_api.routers.databases.schema.escape_identifier"
        ) as mock_escape:
