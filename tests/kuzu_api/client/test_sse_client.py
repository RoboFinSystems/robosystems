...
from robosystems.kuzu_api.client.sse_client import (
...
class TestKuzuIngestionSSEClient:
  ...
  async def test_start_and_monitor_ingestion_success(self, sse_client):
    ...
    with patch(
      "robosystems.kuzu_api.client.sse_client.httpx.AsyncClient"
      ...
      with patch("robosystems.kuzu_api.client.sse_client.aconnect_sse") as mock_connect:
  ...
  async def test_monitor_via_sse_progress_logging(self, sse_client):
    ...
    with patch(
      ...
      with patch("robosystems.kuzu_api.client.sse_client.aconnect_sse") as mock_connect:
        ...
        with patch("robosystems.kuzu_api.client.sse_client.time.time") as mock_time:
...
class TestMonitorIngestionSync:
  ...
  def test_monitor_ingestion_sync_success(self):
    ...
    with patch("robosystems.kuzu_api.client.sse_client.asyncio.run") as mock_run:
