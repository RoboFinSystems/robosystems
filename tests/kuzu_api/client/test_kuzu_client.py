"""Tests for async Kuzu API client."""
...
from robosystems.kuzu_api.client.client import KuzuClient
from robosystems.kuzu_api.client.config import KuzuClientConfig
...
class TestKuzuClient:
  """Test cases for async KuzuClient."""

  @pytest.fixture
  def mock_env(self):
    """Mock environment variables."""
    with patch("robosystems.config.env") as mock:
      mock.KUZU_API_KEY = None
  ...
  async def test_client_context_manager(self, mock_env):
    ...
    async with KuzuClient(base_url="http://localhost:8001") as client:
      assert isinstance(client, KuzuClient)
      assert isinstance(client.client, httpx.AsyncClient)
  ...
  async def test_client_with_custom_config(self, mock_env):
    ...
    client = KuzuClient(config=config)
  ...
  async def test_execute_with_retry_transient_error(self, mock_env):
    ...
    async def mock_func():
      ...
      attempt_count += 1
      if attempt_count < 3:
        raise KuzuTransientError("Temporary failure")
      return {"success": True}
  ...
  async def test_execute_with_retry_syntax_error_no_retry(self, mock_env):
    ...
    async def mock_func():
      nonlocal attempt_count
      attempt_count += 1
      raise KuzuSyntaxError("Invalid query syntax")
  ...
  async def test_execute_with_retry_timeout_conversion(self, mock_env):
    """Test conversion of httpx timeout to KuzuTimeoutError."""
    client = KuzuClient(base_url="http://localhost:8001")

    async def mock_func():
      raise httpx.TimeoutException("Request timed out")

    with pytest.raises(KuzuTimeoutError):
      await client._execute_with_retry(mock_func)
  ...
  async def test_execute_with_retry_max_attempts(self, mock_env):
    ...
    async def mock_func():
      nonlocal attempt_count
      attempt_count += 1
      raise KuzuTransientError("Always fails")
