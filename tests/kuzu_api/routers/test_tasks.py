...
from robosystems.kuzu_api.core.task_sse import TaskType
...
class TestTasksRouter:
  ...
  async def test_list_tasks_all(
    ...
    with patch(
      "robosystems.kuzu_api.routers.tasks.unified_task_manager"
    ) as mock_manager:
  ...
  def test_monitor_task_endpoint_exists(self, client):
    ...
    with patch(
      ...
      with patch("robosystems.kuzu_api.routers.tasks.EventSourceResponse") as mock_sse:
        mock_sse.return_value = MagicMock()

        with patch("robosystems.kuzu_api.routers.tasks.generate_task_sse_events"):
  ...
  async def test_unified_task_manager_get_task_from_redis(self):
    ...
    from robosystems.kuzu_api.routers.tasks import UnifiedTaskManager
