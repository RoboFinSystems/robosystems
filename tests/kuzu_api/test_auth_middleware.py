"""Tests for kuzu_api auth middleware."""
...
from robosystems.kuzu_api.middleware.auth import (
  KuzuAuthMiddleware,
...
class TestKuzuAuthMiddleware:
  ...
  async def test_middleware_development_bypass(self, mock_app, mock_request):
    ...
    with patch("robosystems.kuzu_api.middleware.auth.env") as mock_env:
  ...
  def test_middleware_initialization_requires_key_in_prod(self, mock_app):
    ...
    with patch("robosystems.kuzu_api.middleware.auth.env") as mock_env:
      ...
      with patch(
        "robosystems.kuzu_api.middleware.auth.get_api_key_from_secrets_manager"
      ) as mock_get_key:
...
class TestSecretsManagerIntegration:
  ...
  @patch("robosystems.kuzu_api.middleware.auth.boto3.client")
...
class TestAPIKeyGeneration:
  ...
  def test_create_api_key(self):
    ...
    with patch(
      "robosystems.kuzu_api.middleware.auth.SecurityAuditLogger"
    ) as mock_logger:
  ...
  @patch("robosystems.kuzu_api.middleware.auth.bcrypt")
