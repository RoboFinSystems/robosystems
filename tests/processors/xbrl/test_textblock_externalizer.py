...
from robosystems.processors.xbrl.textblock_externalizer import TextBlockExternalizer
...
class TestGenerateS3Key:
  ...
  def test_generate_s3_key_without_report(self, mock_s3_client, entity_data):
    ...
    with patch(
      "robosystems.processors.xbrl.textblock_externalizer.datetime"
    ) as mock_datetime:
...
class TestQueueValueForS3:
  ...
  def test_queue_value_exception(self, mock_s3_client, entity_data, report_data):
    with patch(
      "robosystems.processors.xbrl.textblock_externalizer.hashlib.sha256"
    ) as mock_sha256:
