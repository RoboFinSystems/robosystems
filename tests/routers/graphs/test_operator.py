...
async def test_agent_router_basic(
  ...
  with (
    ...
    # Mock the OperatorOrchestrator class instance
    ...
    response = client_with_mocked_auth.post(
      f"/v1/graphs/{VALID_TEST_GRAPH_ID}/operator", json={"message": "simple query"}
    )
...
class TestOperatorUnauthorizedAccess:
  ...
  def test_unauthorized_access(self):
    ...
    try:
      ...
      with TestClient(app) as test_client:
        # Test POST /v1/graphs/{graph_id}/operator endpoint
        response = test_client.post(
          f"/v1/graphs/{VALID_TEST_GRAPH_ID}/operator", json={"message": "test"}
        )
...
class TestGraphIdValidation:
  ...
  def test_graph_id_validation_through_path(self, client_with_mocked_auth: TestClient):
    ...
    with (
      ...
      response = client_with_mocked_auth.post(
        "/v1/graphs/ab/operator", json={"message": "test"}
      )
      ...
      try:
        response = client_with_mocked_auth.post(
          "/v1/graphs/test@graph/operator", json={"message": "test"}
        )
