...
class TestGetConnectionOptions:
  ...
  @pytest.mark.unit
  @pytest.mark.asyncio
  async def test_all_providers_enabled_returns_three_providers(self):
    """All three providers enabled → three providers in response."""
  ...
  @pytest.mark.unit
  @pytest.mark.asyncio
  async def test_only_plaid_enabled(self):
    """Only Plaid enabled → single Plaid provider in response."""
    mock_user = _make_mock_user()
    mock_env = _make_mock_env(
    ...
    assert result.total_providers == 1
    assert result.providers[0].provider == "plaid"

  @pytest.mark.unit
  @pytest.mark.asyncio
  async def test_sec_disabled_quickbooks_and_plaid_enabled(self):
    """SEC disabled, QB and Plaid enabled → two providers without SEC."""
    mock_user = _make_mock_user()
    mock_env = _make_mock_env(
      sec_enabled=False, quickbooks_enabled=True, plaid_enabled=True
    )
    ...
    provider_names = [p.provider for p in result.providers]
    assert "sec" not in provider_names
    assert "quickbooks" in provider_names
    assert "plaid" in provider_names

  @pytest.mark.unit
  @pytest.mark.asyncio
  async def test_quickbooks_disabled_sec_and_plaid_enabled(self):
    """QuickBooks disabled, SEC and Plaid enabled → two providers without QB."""
    mock_user = _make_mock_user()
    mock_env = _make_mock_env(
      sec_enabled=True, quickbooks_enabled=False, plaid_enabled=True
    )
    ...
    assert result.total_providers == 2
    provider_names = [p.provider for p in result.providers]
    assert "quickbooks" not in provider_names
    assert "sec" in provider_names
    assert "plaid" in provider_names

  @pytest.mark.unit
  @pytest.mark.asyncio
  async def test_plaid_disabled_sec_and_quickbooks_enabled(self):
    """Plaid disabled, SEC and QB enabled → two providers without Plaid."""
    mock_user = _make_mock_user()
    mock_env = _make_mock_env(
      sec_enabled=True, quickbooks_enabled=True, plaid_enabled=False
    )
  ...
  @pytest.mark.unit
  @pytest.mark.asyncio
  async def test_provider_ordering_is_sec_then_qb_then_plaid(self):
    """Providers appear in canonical order: SEC, QuickBooks, Plaid."""
    mock_user = _make_mock_user()
    mock_env = _make_mock_env(
      sec_enabled=True, quickbooks_enabled=True, plaid_enabled=True
    )
    ...
    provider_names = [p.provider for p in result.providers]
    assert provider_names == ["sec", "quickbooks", "plaid"]
  ...
  @pytest.mark.unit
  @pytest.mark.asyncio
  async def test_plaid_provider_metadata(self):
    """Plaid provider includes link auth_type and bank transaction features."""
    mock_user = _make_mock_user()
    mock_env = _make_mock_env(
      sec_enabled=False, quickbooks_enabled=False, plaid_enabled=True
    )
    ...
    plaid = result.providers[0]
    assert plaid.provider == "plaid"
    assert plaid.display_name == "Bank Connections (Plaid)"
    assert plaid.auth_type == "link"
    assert "bank_transactions" in plaid.features
    assert plaid.documentation_url is not None
  ...
  @pytest.mark.unit
  @pytest.mark.asyncio
  async def test_plaid_has_entity_id_in_required_config(self):
    """Plaid provider requires entity_id configuration."""
    mock_user = _make_mock_user()
    mock_env = _make_mock_env(
      sec_enabled=False, quickbooks_enabled=False, plaid_enabled=True
    )
    ...
    plaid = result.providers[0]
    assert "entity_id" in plaid.required_config
