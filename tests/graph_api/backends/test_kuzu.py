...
def mock_path_mkdir():
  with patch("robosystems.graph_api.backends.kuzu.Path.mkdir"):
    yield


class TestKuzuBackendInitialization:
  @patch("robosystems.graph_api.backends.kuzu.get_connection_pool")
  def test_backend_initialization(self, mock_get_pool):
    mock_pool = MagicMock()
    mock_get_pool.return_value = mock_pool

    backend = KuzuBackend(data_path="/test/path")
  ...
  def test_backend_initialization_default_path(self, mock_get_pool):
    ...
    assert backend.data_path == Path("/data/kuzu-dbs")
    assert backend.connection_pool == mock_pool


class TestKuzuBackendQueryExecution:
  @pytest.mark.asyncio
  @patch("robosystems.graph_api.backends.kuzu.get_connection_pool")
...
class TestKuzuBackendDatabaseManagement:
  @pytest.mark.asyncio
  @patch("robosystems.graph_api.backends.kuzu.get_connection_pool")
  ...
  @pytest.mark.asyncio
  @patch("robosystems.graph_api.backends.kuzu.shutil")
  @patch("robosystems.graph_api.backends.kuzu.get_connection_pool")
  ...
  async def test_list_databases(self, mock_get_pool):
    ...
    mock_file1.stem = "db1"
    mock_file1.suffix = ".kuzu"

    mock_file2 = MagicMock()
    mock_file2.stem = "db2"
    mock_file2.suffix = ".kuzu"
...
class TestKuzuBackendDatabaseInfo:
  @pytest.mark.asyncio
  @patch("robosystems.graph_api.backends.kuzu.get_connection_pool")
...
class TestKuzuBackendS3Ingestion:
  @pytest.mark.asyncio
  @patch("robosystems.graph_api.backends.kuzu.get_connection_pool")
...
class TestKuzuBackendClusterTopology:
  @pytest.mark.asyncio
  @patch("robosystems.graph_api.backends.kuzu.get_connection_pool")
  async def test_get_cluster_topology(self, mock_get_pool):
    ...
    assert result.leader == {"backend": "kuzu"}
