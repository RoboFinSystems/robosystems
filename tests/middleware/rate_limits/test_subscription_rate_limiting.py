...
class TestSubscriptionRateLimits:
  ...
  def test_get_subscription_rate_limit(self):
    ...
    # Standard tier
    limit, window = get_subscription_rate_limit("standard", EndpointCategory.GRAPH_READ)
    assert limit == 500
    assert window == 60

    # Standard tier write
    limit, window = get_subscription_rate_limit(
      "standard", EndpointCategory.GRAPH_WRITE
    )
    assert limit == 100
    assert window == 60
    ...
    # Enterprise tier
    limit, window = get_subscription_rate_limit(
      "enterprise", EndpointCategory.GRAPH_QUERY
    )
    assert limit == 1000
    assert window == 60
  ...
  def test_standard_tier_has_appropriate_limits(self):
    ...
    for category in EndpointCategory:
      standard_limit = get_subscription_rate_limit("standard", category)
...
class TestSubscriptionAwareRateLimiting:
  ...
  def test_subscription_rate_limiting_standard_tier(
    ...
    assert mock_request.state.rate_limit_tier == "standard"
  ...
  def test_mcp_endpoint_category(self):
    ...
    limit, window = get_subscription_rate_limit("standard", EndpointCategory.GRAPH_MCP)
    assert limit == 100
    assert window == 60
