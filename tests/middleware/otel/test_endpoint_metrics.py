...
class TestRecordAuthAttempt:
    ...
    def test_with_user_id(self, metrics):
        metrics.record_auth_attempt(
            "/login", "POST", "api_key", True, user_id="user1"
        )
...
class TestQueryQueueMetrics:
    ...
    def test_execution_failed(self, metrics):
        metrics.record_query_execution(
            "kg123", "u1", 0.5, "failed", error_type="timeout"
        )
...
class TestRecordQueryQueueMetrics:
    def test_submission(self):
        record_query_queue_metrics(
            "submission", "kg123", "u1", priority=3, success=True
        )
    ...
    def test_execution(self):
        record_query_queue_metrics(
            "execution", "kg123", "u1",
            execution_time_seconds=2.0, status="completed",
        )

    def test_concurrent_update(self):
        record_query_queue_metrics(
            "concurrent_update", "kg123", "u1", delta=1
        )


class TestEndpointMetricsContext:
    def test_success_path(self):
        with endpoint_metrics_context("/test", "POST", user_id="u1") as ctx:
            pass  # Success

    def test_failure_path(self):
        with pytest.raises(ValueError):
            with endpoint_metrics_context("/test", "GET") as ctx:
