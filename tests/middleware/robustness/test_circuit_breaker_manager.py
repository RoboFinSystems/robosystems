...
class TestRecordFailureErrorClassification:
  ...
  def test_no_error_increments_failure_count(self):
    ...
    assert cb.get_circuit_status("sec", "cypher_query")["failure_count"] == 1
  ...
  def test_client_error_does_not_increment(self):
    ...
    assert cb.get_circuit_status("sec", "cypher_query")["failure_count"] == 0
  ...
  def test_client_errors_do_not_open_circuit(self):
    ...
    for _ in range(20):
      cb.record_failure("sec", "cypher_query", error=error)
    assert cb.get_circuit_status("sec", "cypher_query")["is_open"] is False
    assert cb.get_circuit_status("sec", "cypher_query")["failure_count"] == 0
  ...
  def test_server_errors_open_circuit_at_threshold(self):
    ...
    for _ in range(5):
      cb.record_failure("sec", "cypher_query", error=error)
    assert cb.get_circuit_status("sec", "cypher_query")["is_open"] is True
  ...
  def test_mixed_errors_only_count_infrastructure(self):
    ...
    assert cb.get_circuit_status("sec", "cypher_query")["failure_count"] == 2
    assert cb.get_circuit_status("sec", "cypher_query")["is_open"] is False
  ...
  def test_circuit_isolation_between_graphs(self):
    ...
    assert cb.get_circuit_status("sec", "cypher_query")["is_open"] is True
    assert cb.get_circuit_status("kg123", "cypher_query")["is_open"] is False
