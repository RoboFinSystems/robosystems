...
class TestLadybugAllocationManager:
  """Test suite for LadybugAllocationManager."""
  ...
  def setup_method(self):
    ...
    self.manager = LadybugAllocationManager(
  ...
  @patch("time.time")
  @patch(
    "robosystems.middleware.graph.allocation_manager.LadybugAllocationManager._publish_allocation_metrics"
  )
  ...
  async def test_allocate_database_no_capacity(self, mock_trigger_scale_up):
    ...
    with pytest.raises(Exception, match="capacity currently available"):
...
class TestLadybugAllocationManagerSubgraphs:
  """Test suite for LadybugAllocationManager subgraph functionality."""
