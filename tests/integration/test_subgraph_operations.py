...
class TestSubgraphOperations:
  ...
  async def test_install_schema_with_extensions(
    self, subgraph_service, mock_kuzu_client
  ):
  ...
  async def test_check_database_has_data(self, subgraph_service, mock_kuzu_client):
    ...
    has_data = await subgraph_service._check_database_has_data(
      mock_kuzu_client, "test_db"
    )
    assert has_data is False

    # Reset mock and test database with data
    mock_kuzu_client.execute.reset_mock()
    mock_kuzu_client.execute.return_value = [{"node_count": 100}]
    has_data = await subgraph_service._check_database_has_data(
      mock_kuzu_client, "test_db"
    )
    assert has_data is True

  async def test_create_backup(self, subgraph_service, mock_kuzu_client):
  ...
  async def test_get_database_stats(self, subgraph_service, mock_kuzu_client):
    ...
    stats = await subgraph_service._get_database_stats(mock_kuzu_client, "test_db")
  ...
  async def test_backup_fallback(self, subgraph_service, mock_kuzu_client):
    """Test backup fallback when method not implemented."""
    # Remove backup method to simulate not implemented
    delattr(mock_kuzu_client, "backup")
