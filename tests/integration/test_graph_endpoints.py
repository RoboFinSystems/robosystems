...
class TestGraphSchemaEndpoints:
    ...
    def test_get_graph_schema(self, client_with_mocked_auth, test_user_graph, sample_graph):
    ...
    def test_validate_schema(self, client_with_mocked_auth, test_user_graph, sample_graph):
...
class TestGraphCreditEndpoints:
    ...
    def test_get_credit_summary(
        ...
        response = client_with_mocked_auth.get(
            f"/v1/graphs/{graph.graph_id}/credits"
        )
        assert response.status_code in [200, 404]
