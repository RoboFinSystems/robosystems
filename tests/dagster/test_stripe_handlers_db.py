...
class TestTerminalStatesStayTerminal:
  ...
  @pytest.mark.asyncio
  async def test_a_pending_cancel_event_does_not_touch_a_failed_subscription(
    self, test_db
  ):
    sub = _subscription(test_db, "failed", ends_at=datetime.now(UTC))
    data = subscription_updated(sub, "active")
    data["cancel_at_period_end"] = True

    await _handle_subscription_updated(data, test_db, MagicMock())

    test_db.expire_all()
    assert test_db.get(BillingSubscription, sub.id).status == "failed"

  @pytest.mark.asyncio
  async def test_a_stale_pre_cancel_event_does_not_reactivate(self, test_db):
    sub = _subscription(
      test_db,
      "canceled",
  ...
  @pytest.mark.asyncio
  @pytest.mark.parametrize("late_status", ["open", "draft", "uncollectible", "void"])
  async def test_a_paid_invoice_stays_paid(self, test_db, late_status):
    org = _org(test_db)
    stripe_invoice = f"in_test_{uuid.uuid4().hex[:10]}"
    invoice = _paid_invoice(test_db, org.id, stripe_invoice)

    await _handle_invoice_updated(
      {"id": stripe_invoice, "object": "invoice", "status": late_status},
      test_db,
      MagicMock(),
    )
