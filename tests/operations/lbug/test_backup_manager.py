"""
Tests for LadybugDB backup manager operations.
...
class TestBackupManager:
  ...
  @pytest.mark.asyncio
  async def test_backup_manager_mocked_lbug(self, backup_manager, sample_cypher_data):
    """Test backup manager with mocked LadybugDB operations."""
    ...
    with (
      ...
      mock_backup_metadata.lbug_version = "0.10.1"
  ...
  async def test_download_backup_functionality(self, backup_manager):
    ...
    assert "database.lbug.zip" in filename
  ...
  async def test_format_conversion_methods(self, backup_manager):
    ...
    assert content_type == "application/zip"
    assert filename == "backup123_database.lbug.zip"
