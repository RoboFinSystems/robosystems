"""Tests for the unified agent system (Agent, AgentContext, TrackedAIClient, adapters)."""
...
from robosystems.operations.agents.ai_client import AIMessage, AIResponse
...
from robosystems.operations.agents.credit_consumer import NoOpCreditConsumer
...
class TestAgentSpec:
  def test_spec_readable_without_instantiation(self):
    assert EchoAgent.spec.name == "Echo Agent"
  ...
  def test_spec_defaults(self):
    ...
    assert AgentMode.STANDARD in spec.supported_modes
...
class TestAgentRun:
  @pytest.mark.asyncio
  async def test_echo_agent(self, agent_context):
    agent = EchoAgent()
    result = await agent.run(agent_context)

    assert result.content == "Test response"
    assert result.metadata["echoed"] is True
    assert agent_context.ai.total_tokens["input"] == 100
  ...
  async def test_default_can_handle(self):
    """Agent base class returns 0.5 by default."""

    class MinimalAgent(Agent):
      spec = AgentSpec(name="Min", description="Minimal", capabilities=[])
...
class TestAgentRegistry:
  ...
  def test_register_and_get(self):
    @register_agent("test_echo")
    class TestEcho(Agent):
      spec = AgentSpec(name="Test", description="Test", capabilities=[])

      async def run(self, ctx):
        return AgentResult(content="ok")

    agent = get_agent("test_echo")
  ...
  def test_list_agents(self):
    ...
    class A(Agent):
      spec = AgentSpec(
        name="A", description="Agent A", capabilities=[AgentCapability.CUSTOM]
      )

      async def run(self, ctx):
        return AgentResult(content="a")
