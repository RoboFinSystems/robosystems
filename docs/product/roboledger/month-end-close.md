...
- **Your own procedures are written down, if you have any.** Ask Claude to save them as a close procedures document. It looks for that document before every close and follows it.
...
2. **It clears the blockers with you.**
   ...
   - *Transactions changed in QuickBooks after they synced.* These are normal. Claude shows you each one, and you choose to restate the month it belongs to, post the difference in an open month, or mark it as already handled. See [When QuickBooks changes after a sync](changes-after-sync.md).
...
A busy month can take longer to close than Claude's call waits for. The close keeps running, and Claude checks back until the receipt is ready. It never needs to be started twice.
...
**Ledger → Closing Book** shows the same picture Claude works from: the last closed month, the month you're working towards, what's blocking it, and each schedule's entry for the month with its amount and status. You can draft entries, close a month and reopen one from here too.
...
- **Run your forecasts again.** The month you closed becomes an actual, and each scenario picks up from its real closing balances. See [Plan and forecast with Claude](plan-and-forecast.md).
