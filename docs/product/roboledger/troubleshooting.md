...
A schedule has an entry due for the month and it hasn't been drafted yet. Ask Claude to draft the month's schedule entries, or use **Draft entry** in **Ledger → Closing Book**. If the blocker names a month long before the one you're closing, a schedule was set up to start before your first open month. Ask Claude to fix that schedule's start. See [Schedules for recurring entries](schedules.md).
...
The Plan page shows a column for every closed month. If your history came over from QuickBooks and you haven't closed month by month in RoboLedger, there are no monthly statements behind it yet. Ask Claude to fill in the plan history. See [Plan and forecast with Claude](plan-and-forecast.md).

## A forecast stops partway

A month failed its checks, and RoboLedger doesn't calculate later months from one that's wrong. Ask Claude which month stopped, which check failed and which lines are involved. The months before it are still there to look at.

## A forecast line didn't follow my assumption

Each line has one owner. If you set a number on a line, a driver or growth rate for the same line steps aside for those months, and Claude lists it as skipped with the reason. A driver also only acts in months you gave it a value for. Other months carry the line forward.

## A forecast has nothing to calculate

Every month of the scenario is now closed, so there's nothing left ahead of it. Ask Claude to make the scenario longer and run it again.

## A one-time item repeats every month in a forecast

Lines you don't mention carry forward from the last actual month, including a one-off. Ask Claude to set that line to zero, or to its usual amount.
