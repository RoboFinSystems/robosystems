---
title: Plan and forecast with Claude
description: Build budgets and what-if scenarios from your closed books. Claude projects all three statements forward month by month, and every month has to balance.
order: 12
section: Work with your books
---

A forecast in RoboLedger is a scenario: a named set of assumptions that Claude projects forward from your last closed month, one month at a time, for up to 36 months. You describe the assumptions in a conversation. RoboLedger calculates the income statement, balance sheet and cash flow for every month, and checks each one the same way it checks your actual books.
...
Tell Claude what you want to plan for. It turns that into a scenario with a name, a kind (budget, forecast or projection, which is a label for you and doesn't change the math), a length in months, and assumptions.
...
Each line has one owner. A line can't be set by you and also grown, or grown while a driver is already moving it. Claude tells you which one is in the way.
...
Carrying forward is a good default and a bad answer for a one-off. If last month had an unusual gain or a big annual payment, ask Claude to set that line to zero, or to its normal amount, so it isn't repeated every month.
...
Then the month is checked with the same rules that check your actual statements: subtotals add up, assets equal liabilities plus equity, the cash flow ties to the balance sheet. If a month fails, the run stops there. Claude tells you which month and which check, and nothing after it is calculated from a month that was wrong.
...
Scenarios are built and changed through Claude. The Plan page is where you read them.

## Change it, and run it again

Ask Claude to change an assumption, and it updates the scenario and runs it again. Running a scenario again replaces its old numbers.

After you close a month, run your scenarios again. The line between actual and forecast moves forward on its own: the month you just closed becomes an actual, and the scenario picks up from its real closing balances. Your assumptions stay as you wrote them. A scenario whose last month is now in the past has nothing left to calculate. Ask Claude to make it longer.

Most scenarios should follow your closes like this. If you want a what-if that stays pinned to an earlier month, such as "what if we had raised prices last January", ask Claude to fix the scenario to that month.
...
- There are four drivers. Anything else is a number you set or a growth rate on a line, so "payroll is 20% of revenue" is something Claude works out and sets month by month, not a rule the scenario keeps.
