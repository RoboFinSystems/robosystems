...
The close posts every draft in the month unless its event has been rejected. An entry waiting here as **Classified** posts at the next close even if nobody approves it, so reject anything that shouldn't post.
...
- **Classified** — its draft entry is already in the ledger, and the next close posts it. You don't have to approve it; reject it if it shouldn't post.
