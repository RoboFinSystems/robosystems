...
class GraphTierConfig:
  ...
  def _load_config(cls) -> dict[str, Any]:
    ...
    try:
      ...
      # print, not logger: circular import.
  ...
  def get_tier_config(cls, tier: str, environment: str | None = None) -> dict[str, Any]:
    """A tier's writer block from graph.yml.

    On a shared replica, the replica's ``instance`` overrides (e.g. memory for
    smaller hardware) are merged over the writer's.
    """
  ...
  def get_duckdb_max_threads(cls, tier: str, environment: str | None = None) -> int:
    """Get DuckDB max threads for a tier (aligned with instance vCPUs)."""
    tier_config = cls.get_tier_config(tier, environment)
    instance_config = tier_config.get("instance", {})
  ...
  def get_api_rate_multiplier(cls, tier: str, environment: str | None = None) -> float:
    """This tier's GRAPH_QUERY limit relative to ladybug-standard.

    Derived from the enforced table so what /limits and /offering advertise
    cannot drift. ``environment`` is unused.
    """
    from .rate_limits import EndpointCategory, RateLimitConfig

    # A tier without its own table is enforced at standard, so 1.0.
  ...
  def get_copy_operation_limits(
    ...
    ``max_file_size_gb`` comes from the enforced MAX_FILE_SIZE_MB, not
    graph.yml: uploads are processed in the shared API container, so the limit
    is the same for every tier.
    """
    tier_config = cls.get_tier_config(tier, environment)
    default_limits = {
      "timeout_seconds": 300,
  ...
  def get_graph_limits(
    ...
    """
    tier_config = cls.get_tier_config(tier, environment)
    # Must track the smallest tier (ladybug-standard in graph.yml): the row
    # caps are OOM guardrails sized to instance RAM.
  ...
  def get_instance_storage_limit_gb(
    ...
    The whole instance's budget (parent graph, subgraphs, DuckDB staging).
    Enforced by materialization and file upload, not advisory.
    """
  ...
  def _generate_tier_features(cls, tier_config: dict[str, Any]) -> list[str]:
    ...
    if is_multitenant:
      ...
      # Physical RAM, as /v1/offering shows; not max_memory_mb (the engine budget).
