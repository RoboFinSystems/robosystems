...
# All scalable ASG groups (tiers + shared-replicas)
ALL_ASG_GROUPS = [*GRAPH_TIERS, "shared-replicas"]

# Mapping from tier/group name to GHA variable prefix
TIER_TO_GHA_PREFIX = {
...
class InstancesHelper:
  ...
  def __init__(self, environment: str, aws_profile: str):
    ...
    # Remove LocalStack endpoint override to use real AWS.
    # Not restored — CLI process is short-lived and boto3 clients
    # resolve endpoints lazily on each API call.
...
def _instance_matches_group(inst: dict, group: str) -> bool:
  ...
  # Exclude shared replicas from writer tier groups
...
def instances_list(client):
  ...
  # Group instances by ASG group.
  # Shared replicas (node_type=shared_replica) get their own group
  # separate from ladybug-shared writers (node_type=shared_master/writer).
...
def instances_info(client, instance_id):
  ...
  # ASG info — shared replicas have their own ASG
  node_type = inst.get("node_type", "")
...
def instances_scale(client, tier, desired, min_size, max_size, force):
  ...
  # Check active graphs to prevent killing instances
  all_instances = helper.get_all_instances()
  tier_instances = [
    inst
    for inst in all_instances
...
def instances_cleanup(client, force):
  ...
  # Build set of instance_ids that have active graphs
  ...
  # Find healthy instances with zero graphs
  empty_instances = []
  for inst in all_instances:
  ...
  # Warn about ASG min constraints
