...
class BackupManager:
  ...
  async def _convert_backup_format(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      if from_format == "csv" and to_format == "json":
        ...
        filename = f"{backup_id}_database.kuzu.zip"
  ...
  def _get_content_type_and_filename(
    ...
    format_map = {
      ...
      "full_dump": ("application/zip", f"{backup_id}_database.kuzu.zip"),
    }
  ...
  async def _convert_full_dump_to_zip(self, dump_file: Path) -> bytes:
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
      # Add the database file with .kuzu extension
      zip_file.write(dump_file, f"database/{dump_file.stem}.kuzu")
  ...
  async def _export_to_csv(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      async with repository:
        # For Kuzu, we need to get schema from our schema loader
        from ...schemas.loader import KuzuSchemaLoader

        try:
          # Get schema information
          schema_loader = KuzuSchemaLoader()
          ...
          for node_type in node_types:
            ...
            try:
              ...
              if count_result and count_result.get("count", 0) > 0:
                # Kuzu COPY TO syntax for exporting
                export_query = f"""
        ...
        try:
          ...
          for node_type in node_types:
            ...
            try:
              ...
              if count_result and count_result.get("count", 0) > 0:
                # Kuzu COPY TO syntax for exporting to Parquet
                # Note: Kuzu uses file extension to determine format, not FORMAT parameter
                export_query = f"""
                          COPY (MATCH (n:{node_type}) RETURN n.*)
                          TO '{parquet_file}'
                          """
          ...
          for node_type in node_types:
            ...
            try:
              ...
              if count_result and count_result.get("count", 0) > 0:
                ...
                # Convert to JSON manually since Kuzu doesn't support direct JSON export
                import json
  ...
  async def _import_full_dump(
      ...
      try:
        ...
        with tempfile.TemporaryDirectory() as backup_temp_dir:
          ...
          if os.path.isfile(target_path):
            ...
            backup_file = backup_temp_path / f"{graph_id}.kuzu.bak"
            shutil.copy2(target_path, backup_file)
            # Remove .bak extension for zip
            final_backup_file = backup_temp_path / f"{graph_id}.kuzu"
          ...
          with zipfile.ZipFile(system_backup_zip, "w", zipfile.ZIP_DEFLATED) as zf:
            if os.path.isfile(target_path):
              ...
              zf.write(final_backup_file, f"{graph_id}.kuzu")
            else:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      if (temp_path / f"{graph_id}.kuzu").exists():
        # Single file database
        shutil.copy2(temp_path / f"{graph_id}.kuzu", target_path)
      else:
        # Directory-based database
