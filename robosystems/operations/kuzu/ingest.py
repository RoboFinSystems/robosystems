...
def ingest_from_s3(
  ...
  try:
    ...
    logger.info(
      f"Starting schema-driven Kuzu ingestion from S3: {bucket}/{s3_prefix} -> {db_name}"
    )
...
def ingest_from_local_files(
  ...
  try:
    ...
    logger.info(f"Starting Kuzu ingestion: {len(file_paths)} files -> {db_name}")

    # Initialize Kuzu engine
    ...
    db_path = get_kuzu_database_path(db_name)
  ...
  try:
    ...
    for field in arrow_schema:
      if field.name in available_columns:
        kuzu_type = _map_arrow_to_kuzu_type(str(field.type))
        ...
        columns.append(f"{column_name} {kuzu_type}")
          ...
          for field in arrow_schema:
            if field.name == prop_name:
              ...
              if column_name.lower() in [
                ...
                column_name = f"`{column_name}`"
              property_columns.append(f"{column_name} {kuzu_type}")
              break
...
def _copy_node_data_schema_driven(
  ...
  """
  Copy node data using schema-driven approach with Kuzu's COPY FROM functionality.
...
def _copy_relationship_data_schema_driven(
  ...
  try:
    ...
    # For relationships in Kuzu, the parquet file must match the table schema exactly.
...
def _map_arrow_to_kuzu_type(arrow_type: str) -> str:
  """Map Arrow types to Kuzu types."""
