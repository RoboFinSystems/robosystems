...
class GenericGraphService:
  ...
  async def create_graph(
    ...
    # Track graph_client for cleanup
    graph_client = None
    ...
    try:
      ...
      logger.error(f"Graph creation failed: {type(e).__name__}: {e!s}")

      # Cleanup on failure - deallocate database if it was allocated
      try:
        if cluster_info:
        ...
        if graph_client:
          logger.info("Closing graph client for failed operation")
          await graph_client.close()
          logger.info("Successfully closed graph client")
