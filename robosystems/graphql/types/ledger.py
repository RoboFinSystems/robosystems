...
from robosystems.models.api.extensions.schedules import (
  ScheduleFactResponse as PydanticScheduleFactResponse,
)
from robosystems.models.api.extensions.schedules import (
  ScheduleFactsResponse as PydanticScheduleFactsResponse,
)
from robosystems.models.api.extensions.schedules import (
  ScheduleListResponse as PydanticScheduleListResponse,
)
from robosystems.models.api.extensions.schedules import (
  ScheduleSummaryResponse as PydanticScheduleSummaryResponse,
)
...
class ScheduleSummary:
  """Single schedule summary (used in list views).

  Hand-written because `entry_template: dict | None` and
  `schedule_metadata: dict | None` need the `JSON` scalar — Strawberry's
  pydantic decorator cannot map untyped `dict` fields.
  """

  structure_id: strawberry.ID
  name: str
  taxonomy_name: str
  entry_template: strawberry.scalars.JSON | None
  schedule_metadata: strawberry.scalars.JSON | None
  total_periods: int
  periods_with_entries: int

  @classmethod
  def from_pydantic(cls, row: PydanticScheduleSummaryResponse) -> ScheduleSummary:
    return cls(
      structure_id=strawberry.ID(row.structure_id),
      name=row.name,
...
class ScheduleList:
  ...
  @classmethod
  def from_pydantic(cls, resp: PydanticScheduleListResponse) -> ScheduleList:
    return cls(schedules=[ScheduleSummary.from_pydantic(s) for s in resp.schedules])


@strawberry.experimental.pydantic.type(
  model=PydanticScheduleFactResponse, all_fields=True
)
class ScheduleFact:
  """Single fact generated by a schedule."""


@strawberry.experimental.pydantic.type(
  model=PydanticScheduleFactsResponse, all_fields=True
)
class ScheduleFacts:
  """All facts for a schedule, optionally filtered by period."""
