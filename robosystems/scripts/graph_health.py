...
class GraphHealthChecker:
        ...
        try:
            ...
            if self.schema["primary_node"]:
                ...
                for key, count in result.data.items():
                    if key != "relationships" and key != "samples" and count is not None and count > 0:
                        primary_count = count
                        break
            else:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "data" in data:
                        ...
                        logger.info(f"    Sample data retrieved ({len(result.data['samples'])} records)")
            except Exception as e:
                result.errors.append(f"Sample query: {str(e)}")
    ...
    def print_report(self):
        ...
        # Samples
        samples = direct.get("data", {}).get("samples") or api.get("data", {}).get("samples")
        if samples:
...
""",
    ...
    parser.add_argument("graph_id", help="Graph/repository ID to check (e.g., 'sec', 'kg123...')")
    parser.add_argument("--api-url", default="http://localhost:8001", help="Graph API URL")
