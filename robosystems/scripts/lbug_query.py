...
# type: ignore
"""
LadybugDB Database Query Tool

Simple command-line tool to execute queries against embedded LadybugDB databases.
...
def execute_query(db_path: str, query: str, format_output: str = "table"):
  """Execute a query against the LadybugDB database."""
  try:
    ...
    if format_output == "table":
      ...
      print_table(results, title="LadybugDB Query Results")
...
def interactive_mode(db_path: str, format_output: str = "table"):
  ...
  print_info_section("📊 Interactive LadybugDB Query Mode")
  print(f"\nDatabase: {db_path}")
  print("\nCommands:")
...
def main():
  parser = argparse.ArgumentParser(
    description="Execute queries against embedded LadybugDB databases",
...
Examples:
  ...
  parser.add_argument(
    "--db-path", required=True, help="Path to the LadybugDB database directory"
  )
