...
@router.patch(
  ...
  description="Set `status=disabled` to exclude a file from ingestion or `status=archived` to soft-delete it. To stage an uploaded file into DuckDB, use `POST /v1/graphs/{graph_id}/operations/ingest-file`.",
  responses={**RESOURCE_ERROR_RESPONSES},
)
@endpoint_metrics_decorator(
  ...
  try:
    ...
    if request.status not in valid_statuses:
      raise HTTPException(
        ...
        f"To stage an uploaded file, use POST /v1/graphs/{{graph_id}}/operations/ingest-file.",
      )
